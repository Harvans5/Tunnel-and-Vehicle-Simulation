...
public abstract class Vehicle implements Runnable {
    ...
    public Condition stopMoving = l.newCondition();
    public Condition ambulance = l.newCondition();
    /**
    ...
    public final void doWhileInTunnel() {
    	long time = (10 - speed) * 100;
    	//If an ambulance is in the tunnel it stops the rest of the vehicle threads in the tunnel
    	...
    	//If the vehicle is not a ambulance
    	 } else {
			  try {
				  l.lock();
				 //Runs while the time is greater than zero meaning the vehicle is still in the tunnel
				 while(time > 0) {
					 l.lock();
					 long start = System.currentTimeMillis();
					 stopMoving.await(time, TimeUnit.MILLISECONDS);
					 long stop = System.currentTimeMillis();
					 //Deducts for the amount of time passed every iteration through the loop
